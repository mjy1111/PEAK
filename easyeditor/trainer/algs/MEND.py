...
class MEND(EditableModel):
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False):
        if 'gpt' in self.config.model_name.lower():
            ...
            #loss1 = self.edit_loss_fn(self.config, outputs, batch["po_target"][0])["nll"]
            #loss = loss +loss1
            #print("outpu logits:", outputs.shape)
            
            #print(loss)
            #print("po_target", batch["po_target"])

            po_nll_loss_each = None
            
            
            #print(self.config)
            for pos in batch["po_target"]:
                if po_nll_loss_each==None:
                    po_nll_loss_each = self.edit_loss_fn(self.config, outputs, pos)["nll"].unsqueeze(0)
                else:
                    po_nll_loss_each = torch.concat((po_nll_loss_each, self.edit_loss_fn(self.config, outputs, pos)["nll"].unsqueeze(0)),dim=0)

            neg_nll_loss_each = None
            for neg in batch["neg_target"]:
                #print(neg)
                if neg_nll_loss_each==None:
                    neg_nll_loss_each = self.edit_loss_fn(self.config, outputs, neg)["nll"].unsqueeze(0)
                else:
                    neg_nll_loss_each = torch.concat((neg_nll_loss_each, self.edit_loss_fn(self.config, outputs, neg)["nll"].unsqueeze(0)),dim=0)
            ...
            margin = 2
            epsilon = 1e-6
            device = neg_nll_loss_each.device

            score_orig = torch.concat((-po_nll_loss_each,-neg_nll_loss_each),dim=0)
            N = score_orig.shape[0]
            score_1 = score_orig.expand(N, N).to(device)       # score_1 shape (N, N)
            score_2 = score_1.transpose(1, 0).to(device)
            ...
            label_1 = label.expand(N, N).to(device)      # label_1 shape (N, N)
            label_2 = label_1.transpose(0, 1).to(device)
            ...
            #print(self.config.aerfa)
            loss = loss + self.config.aerfa*hinge_loss
            #print(self.config.aerfa)
            #loss = loss + 0.0005*hinge_loss

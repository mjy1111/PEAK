...
class KnowledgeNeurons:
    ...
    def modify_weights(
        ...
        for pos in positive_list:
            _, _, pos_label = self._prepare_inputs(prompt, pos)
            positive_labels.append(pos_label)

        for neg in negtive_list:
            _, _, neg_label = self._prepare_inputs(prompt, neg)
            negtive_labels.append(neg_label)
        
        print(positive_list, negtive_list)
        ...
        if mode == "edit":
            ...
            for pos_label in positive_labels:
                positive_embeddings.append(word_embeddings_weights[pos_label])
    
            for neg_label in negtive_labels:
                negtive_embeddings.append(word_embeddings_weights[neg_label])
        ...
        for layer_idx, position in neurons:
            ...
            if mode == "edit":
                if self.model_type == "gpt2":
                    ...
                    #**************neg&pos embeddings add&minus*************    
                    for positive_embedding in positive_embeddings:
                        if positive_embedding.ndim > 1:
                            for te in positive_embedding:
                                output_ff_weights[position, :] += te
                        else:
                            output_ff_weights[position, :] += positive_embedding * 2

                    for negtive_embedding in negtive_embeddings:
                        if negtive_embedding.ndim > 1:
                            for te in negtive_embedding:
                                output_ff_weights[position, :] -= te
                        else:
                            output_ff_weights[position, :] -= negtive_embedding * 2

...
class CounterFactDataset(Dataset):
    ...
    def collate_gpt_fn(self, batch):
        src = [b["requested_rewrite"]["prompt"] for b in batch]
        trg = [b["requested_rewrite"]["target_new"]["str"] for b in batch]
        cond = ["{} >> {} || {}".format(b["requested_rewrite"]["target_true"]["str"],
                                        b["requested_rewrite"]["target_new"]["str"],
                                        b["requested_rewrite"]["prompt"]) for b in batch]
        rephrase = [b["paraphrase_prompts"][0] for b in batch]
        loc = [b["neighborhood_prompts"][0] for b in batch]
        loc_ans = [b["requested_rewrite"]["target_true"]["str"] for b in batch]
